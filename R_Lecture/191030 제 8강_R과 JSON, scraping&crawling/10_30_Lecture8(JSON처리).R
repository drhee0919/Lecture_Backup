...
#(2) user rating , genre출력시키기 
res <- data.frame() #결과를 취합할 df선언 
for(i in 1:length(title)) { 
  url_basic <- "https://www.rottentomatoes.com" #(link와 붙일)기본 url 선언 
  request_url <- str_c(url_basic, link[i]) #요청 url 대입 (기본+링크)
  html_moviepage <- read_html(request_url) #해당 영화페이지 요청 
  nodes_movie <- html_nodes(html_moviepage, #영화평점 획득을 위한 html 경로탐색
                            "strong.mop-ratings-wrap__text--small")
  user_Rating <- html_text(nodes_movie, trim = T)[2] #관람객 평점 대입 
  nodes_genre <- html_nodes(html_moviepage, 
                            "ul>li:nth-child(2)>div:nth-child(2)")
  genre <- html_text(nodes_genre, trim = T)[2] #경로에서 장르명시부분 구체화 
  genre <- str_remove_all(genre, "\n")
  genre <- str_replace_all(genre," ","")
  genre <- str_replace_all(genre,",","&")
  
  df_temp <- data.frame(user_Rating,genre)
...
View(res)
#외부파일로 결과 출력하기 
write.xlsx(res, "C:/R_Lecture/data/rotten_tmt.xlsx")
